...
化けの種類・実測・既定を enforce=False にした理由は known_issues A-1。

設計上の注意:
  * ノルム 0 のクォータニオンだけは enforce に関係なく直前の有効値を返す
    (閾値の問題ではなく正規化が定義できない)
  * stale に達したら跳躍チェックだけを解除する。姿勢基準そのものが飛ぶと、
    飛ぶ前の値と比べ続ける限り永久に復帰できないため
"""
