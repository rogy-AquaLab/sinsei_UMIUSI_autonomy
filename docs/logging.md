...
| ノードのログ | `/tmp/umiusi_logs/` (`$UMIUSI_LOGDIR`) | 小さい |
...
高度 1 m・速度 0.17 m/s で、下向き 800x600 のフローは **約 118 px/s**。
つまり**ブレ [px] ≒ 118 × 露光時間 [s]**:

> **この 118 px/s は `[未検証]` の仮定に乗っています。** ① 下向きカメラの水平画角を 60°
> (= `fx ≈ 693 px`) と仮定、② 到達可能速度を `v_max ≈ 0.68 × max_duty` (sim 実測) と仮定、
> ③ 高度 1 m。**実レンズの画角は測っていません** — 前カメラですら `fovy 60` は sim の値で
> 未検証です (A-14)。桁は動かないので露光の方針 (自動露光では足りない) は変わりませんが、
> **数字そのものを較正値として使わないこと。** 実測は次回のプールで取ります。
...
4. 対地速度をフローで測るつもりなら **`--flow` を付ける**。下カメラの mp4 (フレーム時刻) と
   露光の実値が残る。**露光は先に 20 ms 以下へ固定**しておくこと (上記「対地速度をフローで…」)
5. ノードログ。`tools/umiusi_stack.sh` が `$UMIUSI_LOGDIR` (既定 **`/tmp/umiusi_logs/`**)
   に残す。**`rl.log` は必ず持ち帰ること** — 起動直後の 2 行でどのポリシーで走ったかが決まる。
   `record_run.sh` が停止時に run の `stack_logs/` へ写すので、正常終了なら自動で付いてくる。
   **`/tmp` なので Pi を再起動すると消える** — 異常終了したら再起動前に手で拾うこと
6. 走行前に `tools/bench_rates.py` を 20 秒回して**その日の周期の記録**を取る
