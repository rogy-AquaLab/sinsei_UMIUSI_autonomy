...
| UI のゲームパッドで旋回し続ける | **動く** | `cmd_target_yaw_mode` を `rate` にして左スティック −0.2 → **0.3 rad/s = 17°/s**（3 秒で −52°）。`absolute` だと **−11° で止まる**。以前ここに載せていた −313°/30 秒 は**旋回レートが UI の送信レートに引きずられていたとき**の値（B-17） |
